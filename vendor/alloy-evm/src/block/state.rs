...
/// Database usable for block execution: [`Database`] + [`DatabaseCommit`] with
/// access to the underlying [`State`] wrapper.
///
/// Unlike upstream's blanket alias over `Database + DatabaseCommit`, this
/// exposes the concrete `State` so executors that manage transition-level
/// bookkeeping can reach it generically.
pub trait StateDB: Database + DatabaseCommit {
    /// Database wrapped by the underlying [`State`].
    type Inner: Database;

    /// Returns the underlying [`State`].
    fn as_state_mut(&mut self) -> &mut State<Self::Inner>;
}

impl<D: Database> StateDB for State<D> {
    type Inner = D;

    fn as_state_mut(&mut self) -> &mut State<D> {
        self
    }
}
...
impl<T: StateDB> StateDB for &mut T {
    ...
    fn as_state_mut(&mut self) -> &mut State<T::Inner> {
        (**self).as_state_mut()
    }
}
